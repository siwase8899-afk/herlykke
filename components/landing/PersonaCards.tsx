...
const personas = [
  {
    ...
    borderColor: 'border-alma-primary',
    benchmark: '폐경이행기 초기',
    journey: {
  ...
  },
  {
    ...
    borderColor: 'border-alma-secondary',
    benchmark: '폐경 후기',
    journey: {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-alma-text-secondary">
            카드를 탭하면 비슷한 분의 <span className="text-alma-accent font-semibold">실제 여정</span>을 볼 수 있어요
          </p>
        </div>
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-5">
                  <div className={`relative w-12 h-12 rounded-full overflow-hidden border-2 transition-colors ${
                    isExpanded ? p.borderColor : 'border-alma-border'
                  }`}>
                    <Image
                      src={p.image}
                      alt={p.name}
                  ...
                  <div>
                    <div className="text-sm font-bold text-alma-text">{p.name} · {p.age}</div>
                    <div className="text-xs text-alma-text-tertiary">{p.role}</div>
                  </div>
                ...
                {!isExpanded && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] text-alma-text-tertiary">
                      여정 보기
                ...
                <div className={`overflow-hidden transition-all duration-500 ${
                  ...
                  <div className="pt-4 border-t border-alma-border space-y-5">
                    {/* BEFORE */}
                    <div className="flex gap-3">
                      <div className="flex flex-col items-center">
                        <div className="w-7 h-7 rounded-full bg-alma-text-tertiary/20 flex items-center justify-center flex-shrink-0">
                          <span className="text-[10px] font-bold text-alma-text-tertiary">B</span>
                        </div>
                        <div className="w-px flex-1 bg-alma-border mt-1" />
                      </div>
                      <div className="pb-2">
                        <p className="text-[11px] font-semibold text-alma-text-tertiary uppercase tracking-wider mb-1">Before</p>
                        <p className="text-sm text-alma-text-secondary leading-relaxed">{p.journey.before}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                      <div className="flex flex-col items-center">
                        <div className="w-7 h-7 rounded-full bg-alma-accent/20 flex items-center justify-center flex-shrink-0">
                          <span className="text-[10px] font-bold text-alma-accent">!</span>
                        </div>
                      </div>
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <p className="text-[11px] font-semibold text-alma-accent uppercase tracking-wider">After {p.journey.duration}</p>
                        </div>
                        <p className="text-sm font-medium text-alma-text leading-relaxed">{p.journey.after}</p>
                      </div>
                    </div>
