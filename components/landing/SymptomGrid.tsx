...
export function SymptomGrid() {
  ...
  if (!mounted) {
    return (
      <section className="px-6 md:px-8 py-24 md:py-32 bg-white border-t border-hlk-border">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16">
            <div className="h-8 bg-hlk-bg rounded w-1/2 mx-auto mb-4" />
            <div className="h-4 bg-hlk-bg rounded w-1/3 mx-auto" />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="h-32 bg-hlk-bg rounded-2xl animate-pulse" />
            ))}
          </div>
        </div>
      </section>
  ...
  return (
    <section className="px-6 md:px-8 py-16 md:py-24 bg-white border-t border-hlk-border">
      <div className="max-w-4xl mx-auto">
        {/* 상단 배지 */}
        <div className="flex justify-center mb-8">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 bg-hlk-accent/10 rounded-full">
            <span className="w-1.5 h-1.5 rounded-full bg-hlk-accent" />
            <span className="text-xs font-bold text-hlk-accent uppercase tracking-wider">함께해요</span>
          </span>
        </div>

        {/* 헤더 */}
        <div className="text-center mb-10">
          <h2 className="text-2xl md:text-3xl font-bold text-hlk-text mb-3">
            오늘 당신의 몸은 뭐라고 했나요?
          </h2>
          ...
          {/* 실시간 카운트 */}
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-hlk-primary-light rounded-full">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-hlk-primary opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-hlk-primary" />
            </span>
            <span className="text-sm font-medium text-hlk-primary">
          ...
          {SYMPTOM_CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                style={isActive ? {
                  backgroundColor: cat.id === 'body' ? 'var(--color-hlk-primary)' : cat.id === 'mind' ? 'var(--color-hlk-accent)' : 'var(--color-hlk-secondary)',
                } : undefined}
              >
                {cat.label}
                {catCount > 0 && (
                  <span className={`w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center ${
                    isActive ? 'bg-white/30 text-white' : 'bg-hlk-primary text-white'
                  }`}>
                    {catCount}
                  </span>
                )}
              ...
              <button
                ...
                className={`group relative text-left rounded-2xl p-5 transition-all duration-300 cursor-pointer ${
                  isEmpathized
                    ...
                    : 'hover:shadow-xl hover:-translate-y-1 hover:border-hlk-primary/20'
                }`}
                style={{
              ...
              >
                ...
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <p className={`text-[11px] font-semibold uppercase tracking-wide ${
                    isEmpathized ? 'text-white/60' : 'text-hlk-text-tertiary'
                  }`}>
                    {char.name}
                  </p>
                </div>
                ...
                <p className={`text-base md:text-lg font-bold leading-snug mb-3 transition-transform duration-300 group-hover:translate-x-0.5 ${
                  isEmpathized ? 'text-white' : 'text-hlk-text'
                }`}>
                  {char.tagline}
                </p>
                ...
                <p className={`text-[11px] leading-relaxed mb-2 max-h-0 opacity-0 group-hover:max-h-12 group-hover:opacity-100 transition-all duration-500 overflow-hidden ${
                  isEmpathized ? 'text-white/70 max-h-12 opacity-100' : 'text-hlk-text-secondary'
                }`}>
                  {char.description}
                </p>
                ...
                <p className={`text-[11px] font-medium ${
                  isEmpathized ? 'text-white/80' : 'text-hlk-text-tertiary'
                }`}>
        ...
        {/* 전체 증상 수 안내 */}
        <p className="text-center text-xs text-hlk-text-tertiary mb-8">
        ...
        {/* 하단 CTA 영역 */}
        {myEmpathyCount > 0 ? (
          <div className="bg-gradient-to-r from-hlk-primary-light to-hlk-accent-light rounded-2xl p-6 text-center">
            <p className="text-lg font-bold text-hlk-text mb-2">
              {myEmpathyCount}개 증상에 <span className="text-hlk-primary">&ldquo;나도!&rdquo;</span> 했어요
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link
                href="/log/new"
                className="inline-flex items-center justify-center px-6 py-3 bg-hlk-accent text-white font-bold rounded-full hover:bg-hlk-accent/90 transition-all shadow-lg shadow-hlk-accent/30"
              >
                오늘 나의 상황 기록하기
              ...
              <Link
                href="/community"
                className="inline-flex items-center justify-center px-6 py-3 bg-white text-hlk-primary font-semibold rounded-full border-2 border-hlk-primary hover:bg-hlk-primary-light transition-all"
              >
                커뮤니티에서 이야기하기
              </Link>
            </div>
          ...
          <div className="text-center">
            <p className="text-sm text-hlk-text-tertiary mb-4">
              증상을 탭해서 공감하거나, 바로 기록을 시작해보세요
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link
                href="/log/new"
                className="inline-flex items-center justify-center px-6 py-3 bg-hlk-primary text-white font-semibold rounded-full hover:bg-hlk-primary-dark transition-all"
              >
                오늘 나의 상황 기록하기
              </Link>
              <Link
                href="/community"
                className="text-hlk-primary font-medium hover:underline py-3"
              >
                커뮤니티 둘러보기 →
              </Link>
            </div>
