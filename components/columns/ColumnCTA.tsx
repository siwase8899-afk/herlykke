...
export function ColumnCTA({ symptomName }: { symptomName: string }) {
  ...
  return (
    <div className="py-10">
      <div className={`rounded-2xl p-8 text-center ${isLoggedIn ? 'bg-alma-accent-light' : 'bg-alma-primary-light'}`}>
        <p className="text-sm text-alma-primary font-semibold mb-2">
          이 글이 도움이 되셨나요?
        </p>
        <h3 className="text-xl font-bold text-alma-text mb-3">
          {isLoggedIn
        ...
        <Link
          ...
          className="inline-block px-8 py-3 bg-alma-accent text-white font-semibold rounded-full hover:bg-alma-accent/90 transition-colors"
        >
