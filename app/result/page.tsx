      ...
      <div className="max-w-lg mx-auto px-6 md:px-8 py-10">
        ...
        <div className="text-center mb-10">
          <div className="relative inline-block">
            ...
            <div className="absolute inset-0 bg-alma-accent/20 rounded-full blur-2xl -z-10" />
          </div>
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-alma-accent-light rounded-full">
            ...
            <span className="text-sm font-semibold text-alma-accent">1일 스트릭 시작!</span>
          </div>
        </div>

        {/* 다음 목표 */}
        <div className="bg-white rounded-2xl p-4 border border-alma-border shadow-sm mb-6">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-alma-primary-light flex items-center justify-center">
              <span className="text-2xl">🎯</span>
            </div>
            <div className="flex-1">
              <p className="text-sm font-bold text-alma-text">다음 목표: 7일 연속 체크인</p>
              <div className="flex items-center gap-2 mt-1">
                <div className="flex-1 h-2 bg-alma-bg rounded-full overflow-hidden">
                  <div className="w-[14%] h-full bg-alma-accent rounded-full" />
                </div>
                <span className="text-xs text-alma-text-tertiary">1/7</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* 추천 솔루션 미리보기 */}
        <div className="mb-8">
          <h2 className="text-lg font-bold text-alma-text mb-3 flex items-center gap-2">
          ...
          <div className="space-y-3">
            {(() => {
              ...
              return solutions.map((sol, i) => (
                <div key={i} className="bg-white rounded-xl p-4 border border-alma-border flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-alma-primary-light flex items-center justify-center flex-shrink-0 text-xl">
                    {sol.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-alma-text">{sol.title}</p>
                    <p className="text-xs text-alma-text-tertiary truncate">{sol.desc}</p>
                  </div>
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {[
              ...
              <Link key={i} href="/community" className="block bg-white rounded-xl p-4 border border-alma-border hover:shadow-sm transition-all">
                <p className="text-sm text-alma-text leading-relaxed line-clamp-2 mb-2">
                  {post.content}
                </p>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-alma-text-tertiary">{post.name}</span>
                  <div className="flex items-center gap-3 text-xs text-alma-text-tertiary">
        ...
        {isLoggedIn ? (
          ...
          <>
            {/* 가입 동기 강화 */}
            <div className="bg-gradient-to-br from-alma-primary to-alma-accent rounded-2xl p-6 mb-6 text-white text-center">
